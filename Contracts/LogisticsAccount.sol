...
contract LogisticsAccount is IAccount
{
    ...
    function execute(address target, uint256 value, bytes calldata data, address signer) external
    {
        ...
        (selector == bytes4(keccak256("logPathHistory(uint256,address,address,string,string)")) || selector == bytes4(keccak256("logCondition(uint256,address,string,int256,string)")) || selector == bytes4(keccak256("changeStatus(uint256,address)"))))
        {
