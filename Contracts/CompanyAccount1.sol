...
contract CompanyAccount is IAccount
{
    ...
    function execute(address target, uint256 value, bytes calldata data, address signer) external
    {
        ...
        (selector == bytes4(keccak256("addProductInfo(address,string,string,string,string,uint256,address)")) || selector == bytes4(keccak256("deleteProductInfo(address,uint256)"))))
        {
            address firstArg = abi.decode(data[4:36], (address));
            require(signer == firstArg, "Employee can add or delete product info under only his own account");
            _call(target, value, data);
        }
